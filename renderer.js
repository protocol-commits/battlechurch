...
(function setupRenderer(window) {
  ...
  function drawSentryTurret(ctx, sentryState) {
    ...
    if (sprite) {
      ...
      ctx.globalCompositeOperation = "lighter";
      ctx.shadowColor = sentryState.glowColor || "rgba(255, 220, 140, 0.9)";
      ctx.shadowBlur = sentryState.glowBlur || 18;
      ctx.globalAlpha = fadeAlpha;
      ctx.drawImage(sprite, -size / 2, -size / 2, size, size);
      ctx.shadowColor = sentryState.glowColor || "rgba(255, 245, 210, 0.9)";
      ctx.shadowBlur = sentryState.glowBlur ? sentryState.glowBlur + 10 : 28;
